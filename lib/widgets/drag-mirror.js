...
function drag_mirror(parent_el, img_1, img_2) {
...
function get_image_dimensions() {
	var top_el = this.parent_el.querySelector('.drag-mirror-top')
...
function image_load_handler(key) {
	return function image_load() {
		image_load_handler.images = image_load_handler.images || {}
		image_load_handler.images[key] = true

		if(Object.keys(image_load_handler.images).length === 2) {
			this.ready()
		}
	}.bind(this)
}
